    ...
    def next_epoch(self):
        ...
        while enabled:
            ...
            for b_i in range(start_idx, self.num_batches):
                ...
                # ---------- 新增过滤逻辑 ----------
                # 保证每个 chunk 长度和预期一致
                expected_len_3d = self.batch_3d.shape[1] if self.poses_3d is not None else None
                expected_len_2d = self.batch_2d.shape[1]
                filtered_chunks = []
                for c in chunks:
                    seq_i, start_3d, end_3d, flip = c
                    chunk_len = end_3d - start_3d
                    if self.poses_3d is not None and chunk_len != expected_len_3d:
                        continue
                    if chunk_len != expected_len_2d and self.poses_3d is None:
                        continue
                    filtered_chunks.append(c)

                if len(filtered_chunks) < self.batch_size:
                    continue  # 跳过不满 batch 的情况

                chunks = filtered_chunks
                # -------------------------------------
                # 如果这个 batch 不满 batch_size，则跳过
                if len(chunks) != self.batch_size:
                    continue  # 跳过这个 batch

                # ---------- 新增过滤逻辑：空 slice / 不足长度 ----------
                valid_chunks = []
                for seq_i, start_3d, end_3d, flip in chunks:
                    # 2D slice 检查
                    low_2d = max(start_3d - self.pad - self.causal_shift, 0)
                    high_2d = min(end_3d + self.pad - self.causal_shift, self.poses_2d[seq_i].shape[0])
                    if high_2d - low_2d <= 0:
                        continue  # 空 slice，舍弃
                    ...
                    if self.poses_3d is not None:
                        ...
                        high_3d = min(end_3d, self.poses_3d[seq_i].shape[0])
                        if high_3d - low_3d <= 0:
                            continue  # 空 slice，舍弃

                    valid_chunks.append((seq_i, start_3d, end_3d, flip))

                # 如果 batch 中有效 chunk 不够，跳过整个 batch
                ...
                for i, (seq_i, start_3d, end_3d, flip) in enumerate(chunks):
                    if end_3d - start_3d != self.chunk_length:
                        continue  # 跳过不完整 chunk
                    ...
                    if high_2d - low_2d <= 0:
                        continue  # slice 长度为0，直接跳过这个 chunk
                    ...
                    if self.poses_3d is not None:
                        ...
                        # ---------------- 新增检查 ----------------
                        if high_3d - low_3d <= 0:
                            continue  # 空 slice，直接跳过这个 chunk
                        # ----------------------------------------
                        pad_left_3d = low_3d - start_3d
                        pad_right_3d = end_3d - high_3d

                        seq_slice = seq_3d[low_3d:high_3d]

                        if seq_slice.size == 0:
                            # 空数据时，用零填充，shape 与预期一致
                            self.batch_3d[i] = np.zeros((end_3d - start_3d, seq_3d.shape[1], seq_3d.shape[2]),
                                                        dtype=seq_3d.dtype)
                        else:
                            if pad_left_3d != 0 or pad_right_3d != 0:
                                self.batch_3d[i] = np.pad(seq_slice,
                                                          ((pad_left_3d, pad_right_3d), (0, 0), (0, 0)),
                                                          'edge')
                            else:
                                self.batch_3d[i] = seq_slice
...
class UnchunkedGenerator:
    ...
    def next_epoch(self):
        for seq_cam, seq_3d, seq_2d in zip_longest(self.cameras, self.poses_3d, self.poses_2d):
            # ---- 新增：基础校验，遇到无效样本直接跳过 ----
            if seq_2d is None or not hasattr(seq_2d, 'ndim') or seq_2d.ndim != 3 or seq_2d.shape[0] == 0:
                print('[WARN][UnchunkedGenerator] skip invalid 2D sequence:',
                      type(seq_2d), getattr(seq_2d, 'shape', None))
                continue

            if seq_3d is not None:
                if not hasattr(seq_3d, 'ndim') or seq_3d.ndim != 3 or seq_3d.shape[0] == 0:
                    print('[WARN][UnchunkedGenerator] skip due to invalid 3D sequence:',
                          type(seq_3d), getattr(seq_3d, 'shape', None))
                    continue

            # 计算安全的 pad（防止出现负 pad）
            left = int(self.pad + self.causal_shift)
            right = int(self.pad - self.causal_shift)
            if left < 0 or right < 0:
                # 你也可以选择直接 continue；这里采取夹紧到 0，尽量不丢样本
                left = max(left, 0)
                right = max(right, 0)
            ...
            # ---- 新增：pad 异常兜底，失败就跳过该样本 ----
            try:
                batch_2d = np.expand_dims(
                    np.pad(seq_2d, ((left, right), (0, 0), (0, 0)), mode='edge'),
                    axis=0
                )
            except Exception as e:
                print('[WARN][UnchunkedGenerator] np.pad failed for 2D:',
                      getattr(seq_2d, 'shape', None), 'pad=', (left, right), e)
                continue
